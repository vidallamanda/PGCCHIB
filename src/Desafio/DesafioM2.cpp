/* Criando Triângulos a partir de um único clique do Mouse
 *
 * Ao clicar com o botão esquerdo do mouse, um triângulo é criado
 * automaticamente na posição do cursor. A geometria base do triângulo
 * é fixa (formato e tamanho), mas sua posição e cor variam.
 * A cor é selecionada ciclicamente a partir de uma paleta.
 * 
 * Um único VAO é reutilizado para todos os triângulos criados.
 * 
...
// VAO global do triângulo base para reutilização
GLuint baseTriangleVAO;
...
int main()
{
    ...
    // Criando o VAO do triângulo base para reutilização
    baseTriangleVAO = createTriangle(
        -10.0f, 10.0f,
        10.0f, 10.0f,
        0.0f, -10.0f);

    // Parte 1: triângulos fixos
    triangles.push_back({vec3(100, 100, 0.0f), colors[iColor++ % colors.size()]});
    triangles.push_back({vec3(200, 100, 0.0f), colors[iColor++ % colors.size()]});
    triangles.push_back({vec3(300, 100, 0.0f), colors[iColor++ % colors.size()]});
    triangles.push_back({vec3(400, 100, 0.0f), colors[iColor++ % colors.size()]});
    triangles.push_back({vec3(500, 100, 0.0f), colors[iColor++ % colors.size()]});
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Desenha triângulos criados com clique (com transformação)
        for (const auto &tri : triangles)
        {
            glBindVertexArray(baseTriangleVAO);

            mat4 model = translate(mat4(1.0f), vec3(tri.position.x, tri.position.y, 0.0f));
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods)
{
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
    {
        ...
        // Cria novo triângulo na posição do clique
        Triangle tri;
        tri.position = vec3((float)xpos, (float)ypos, 0.0f);

        // Gera uma cor da paleta rotativa
